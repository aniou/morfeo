...
Emulator itself is splitted between two parts - a frontend (GUI) 
and backend (machine itself). Usually a GUI part relies on the
internals of machine itself, for example on bitmap buffers that
...
communicate with devices via bus but a devices can not communiacte
...
``ra``   - Requested Address, address issued by CPU or user to BUS
...
``ea``   - Effective Address, addres after all calulations at BUS
           level. 
``addr`` - an internal addres of device, usually substraction between
...
``out``  - returned value, usually from write() routines
``val``  - argument value, usuwally for  read() routines
