...
render_gui :: proc(p: ^platform.Platform) {
        ...
        // Step 3.5: cursor blink
        if time.tick_since(gui.gpu0.cursor_last_tick) >= gui.gpu0.cursor_rate_ms {
            gui.gpu0.cursor_visible   = !gui.gpu0.cursor_visible
            gui.gpu0.cursor_last_tick = time.tick_now()
        }
        ...
        if time.tick_since(gui.gpu0.last_tick) >= gui.gpu0.delay {
            ...
            p.bus.timer1->tick(1)   // id doesn't matter at this moment
        }
