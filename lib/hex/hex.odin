...
// taken from original, not-finished-yet core lib of Odin
...
// change from original: src from []u8 to string
decode :: proc(src: string, allocator := context.allocator) -> (dst: []byte, ok: bool) #no_bounds_check {
	if len(src) % 2 == 1 {
		return
	}
