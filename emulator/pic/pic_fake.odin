...
make_pic_fake    :: proc(name: string) -> ^PIC {
    pic           := new(PIC)
    pic.name       = name
...
read_pic_fake    :: proc(pic: ^PIC, mode: MODE, addr, ra: u32) -> (out: u32) { return }
write_pic_fake   :: proc(pic: ^PIC, mode: MODE, addr, ra, val: u32)          {        }
