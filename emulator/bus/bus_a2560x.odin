...
write_a2560x :: proc(bus: ^BUS_A2560X, mode: MODE, ra, val: u32) {
    ...
    case 0x00_A0_0000 ..= 0x00_BF_FFFF:  emu.error_write(bus.name, s.req, .NOT_IMPL, mode, ra - 0x00_A0_0000, val) // vram1
    ...
    case 0xFE_C0_0000 ..= 0xFE_C1_FFFF:  emu.error_write(bus.name, s.req, .NOT_IMPL, mode, ra - 0xFE_C0_0000, val)
    ...
    case 0xFF_C0_0000 ..= 0xFF_FF_FFFF:  emu.error_write(bus.name, s.req, .NOT_IMPL, mode, ra - 0xFF_C0_0000, val)
    case                              :  emu.error_write(bus.name, s.req, .NOT_IMPL, mode, ra, val)
    }
