...
c256_dma_read8 :: #force_inline proc(bus: ^BUS_C256, kind: DMATYPE, addr: u32) -> (val: u32) {
    switch kind {
    case .SRAM:    val = bus.ram0->read(.mode_8, addr)
    case .VRAM:    val = bus.gpu0->read(.mode_8, addr, .VRAM0)
    }
    return
}
